...
class MyEmbed(discord.Embed):
    ...
    def to_dict(self):
        if "name" not in constants.EMBED_FOOTER or "value" not in constants.EMBED_FOOTER:
            self.addFooterField = False
        if constants.EMBED_FOOTER["name"] == "" or constants.EMBED_FOOTER["value"] == "":
            self.addFooterField = False

        if "inline" in constants.EMBED_FOOTER:
            inline = constants.EMBED_FOOTER["inline"]
        else:
            inline = True

        if self.addFooterField:
            self.add_field(name=constants.EMBED_FOOTER["name"], value=constants.EMBED_FOOTER["value"], inline=inline)
